...
| QUE-06 | Продвижение очереди: гашение истёкших прав и выдача новых происходит в одной транзакции с запросом статуса от ожидающих (право всегда переходит первому в очереди) |

**Право на покупку**

| ID | Требование                                                                                                                                    |
|---|-----------------------------------------------------------------------------------------------------------------------------------------------|
| RGT-01 | Выдача права атомарна: число активных прав никогда не превышает числа доступных единиц (count(active granted) + count(purchased) <= stock)    |
| RGT-02 | Право временное, срок хранится полем `expires_at` в БД (3-5 минут наверное оптимально, можно отдельный полем задавать в конфиге)              |
...
| RGT-05 | Право непередаваемое: другой пользователь не может им воспользоваться, даже получив токен, так как он проверяется с user_id                   |
...
| A-09 | Товар разобран, пока пользователь ждал | Состояние `sold_out` (count(purchased) == stock) с предложением альтернатив            |
...
**Backend:** Go · gin · pgx · миграции goose  
